...
from wechat import wesdk as query
...
class Bot(threading.Thread):
    ...
    def wshd_recv_txt_msg(self, msg):
        ...
        if msg['content'] in key_replys:
            self.ws.send(query.send_msg(key_replys[msg['content']], wxid=msg['senderid'], roomid=msg['roomid'],
                                        nickname=msg['nickname']))
        for f in func_replays:
            if not f["match"](msg['content']):
                continue
            self.ws.send(query.send_msg(f["replay"](msg['content']), wxid=msg['senderid'], roomid=msg['roomid'],
                                        nickname=msg['nickname']))
            break
